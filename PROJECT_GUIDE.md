...
## 8. 🛡️ لوحة التحكم والإدارة السرية ونظام المصادقة الثنائية (Admin Portal & 2FA)

تم تزويد الموقع بـ **لوحة إدارة ومتابعة سرية متكاملة** تعمل داخل نفس التطبيق ومحمية بنظام أمني من خطوتين (**2-Factor Authentication**).

### 🔑 كيفية فتح لوحة التحكم:
افتح رابط الموقع وأضف المسار السري في نهاية الرابط (أو اضغط على الرابط المباشر):
* محلياً (Local): **`http://localhost:5175/#admin-secret`**
* بعد النشر (Live): **`https://hamdyKouta1.github.io/invitation/#admin-secret`**

---

### 🔐 خطوات تسجيل الدخول بالمصادقة الثنائية (2FA):

1. **الخطوة الأولى (Email & Password):**
   * **البريد الإلكتروني:** البريد المحدد في ملف `.env` (`VITE_ADMIN_EMAIL`)
   * **كلمة المرور:** كلمة المرور المحددة في ملف `.env` (`VITE_ADMIN_PASSWORD`)
2. **الخطوة الثانية (Email OTP Code):**
   * يتم توليد رمز تحقق سري من 6 أرقام وإرساله إلى بريدك الإلكتروني.
   * إدخال الرمز المكون من 6 أرقام للدخول الفوري.

---

### 🌟 مميزات لوحة التحكم:

1. **👥 إدارة الضيوف وتأكيدات الحضور (Guests & RSVP):**
   * عرض إجمالي الردود، المؤكد حضورهم، المعتذرون، وإجمالي عدد الحاضرين شاملاً المرافقين.
   * إضافة ضيف جديد، تعديل بيانات أي ضيف، وحذف الضيوف مباشرة من قاعدة بيانات **Firestore DB**.
   * البحث وتصفية الضيوف (سأحضر / اعتذر / ترك رسالة تهنئة).
   * **تصدير كامل للبيانات** بضغطة زر إلى ملف **Excel/CSV** أو ملف **JSON**.

2. **📊 التحليلات والإحصائيات المباشرة (Analytics):**
   * نسبة قبول الدعوة (Acceptance Rate %).
   * توزيع الحضور والمرافقين وإحصائيات حائط التهاني.

3. **📸 إدارة ورفع صور المعرض (Photo Gallery Manager):**
   * إمكانية رفع الصور مباشرة من جهازك (JPG / PNG / WEBP) أو إدخال روابط Google Drive / URLs.
   * تحديد أبعاد الصورة (طولية / عرضية / مربعة) ووصفها، وحفظها مباشرة في قاعدة بيانات المعرض `gallery`.
   * حذف وإعادة ترتيب الصور.

4. **⚙️ تعديل إعدادات الزفاف المباشرة (Live Config Editor):**
   * تعديل أسماء العروسين، العائلات، الآباء والأمهات، موعد الحفل، نصوص العرض، مكان القاعة، رابط الخرائط، وملف الموسيقى.
   * زر **"حفظ في Firestore DB"**: يقوم بتحديث الإعدادات مباشرة في سحابة Firebase لتنعكس فوراً على موقع الدعوة دون الحاجة لإعادة نشر الكود!
   * زر **"تحميل weddingConfig.js"**: لتحميل نسخة كود محدثة لوضعها في ملفات المشروع البرمجية.

---

✨ **بهذا يصبح لديك نظام متكامل لإدارة ومتابعة حفل الزفاف بأعلى درجات الأمان والسهولة!**
